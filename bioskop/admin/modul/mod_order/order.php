  ...
  default:
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr bgcolor=$warna>
				...
				if($r[status_pesanan]=='Lunas'){
					echo "<center><input type=button value='Detail' onclick=\"window.location.href='?module=order&act=detailorder&id=$r[id_orders]';\">";
					echo "<input style='width:80px;' type=button value='Cetak' onclick=\"window.location.href='print.php?id=$r[id_orders]';\">";
				echo "</center></td>";
				}elseif($r[status_pesanan]=='Menunggu Pembayaran'){
					echo "<center>Menunggu Pembayaran
					
				
				</td>";
				}elseif($r[status_pesanan]=='Menunggu Konfirmasi'){
					echo "<center><input type=button value='Konfirmasi' onclick=\"window.location.href='?module=order&act=konfirmasi&id=$r[id_orders]';\">
...
echo "<table style='margin-bottom:7%;' width='100%'><br/>
    ...
    break;  
     ...
     case "konfirmasi":
$query = mysql_query("SELECT * FROM konfirmasi left join rekening on konfirmasi.id_rekening=rekening.id_rekening 
join laporan on laporan.id_orders=konfirmasi.id_orders
join movie on movie.id_movie=laporan.id_movie
join users on users.username=laporan.username
where konfirmasi.id_orders=$_GET[id]");
$r=mysql_fetch_array($query);
...
echo "<div class='post_title'><b>Detail Konfirmasi Pembayaran Tiket untuk Film : $r[judul].</b></div><br/>";
echo "<table style='margin-bottom:7%;' width='100%'><br/>
          <tr><td width='15%'>Nama Pemesan</td>     	<td> : &nbsp;<input type=text name='judul' value='$r[nama_lengkap]' size=49 class='input3' readonly='on'></td></tr>
		  <tr><td>Total Bayar</td>     	<td> : &nbsp;<input type=text name='judul' value='$r[total_bayar]' size=49 class='input3' readonly='on'></td></tr>
		  <tr><td>No Rek</td>     	<td> : &nbsp;<input type=text name='judul' value='$r[rek_anda]' size=49 class='input3' readonly='on'></td></tr>
		  <tr><td>Atas Nama<br/></td>     	<td> : &nbsp;<input type=text name='judul' value='$r[atas_nama]' size=49 class='input3' readonly='on'></td></tr>
													 
			
		  <tr><td>Nama bank</td>     	<td> : &nbsp;<input type=text name='judul' value='$r[nama_bank]' size=59 class='input2' readonly='on'></td></tr>
		  <tr><td>Pesan</td>   	<td> : &nbsp;<input type=text name='studio' value='$r[pesan]' size=59 class='input2' readonly='on'></td></tr>
		  </tr>
		  
	  </table>
	  <center><input style='width:200px; padding:8px;' type=button value='Konfirmasi Status Lunas' onclick=\"window.location.href='$aksi?module=order&act=konfirmasi&id=$r[id_orders]';\"></br></br></br></center>";
    break;  
}
?>
