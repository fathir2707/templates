    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr bgcolor=$warna>
				...
				if($r[status_pesanan]=='Lunas'){
					echo "<center><input type=button value='Detail' onclick=\"window.location.href='detail-pemesanan-$r[id_orders].html';\">";
					...
					echo "<center><input type=button value='Konfirmasi Pembayaran' onclick=\"window.location.href='konfirmasi-$r[id_orders].html';\">
					
				
				</td>";
				}elseif($r[status_pesanan]=='Menunggu Konfirmasi'){
					echo "<center>Menunggu Konfirmasi
...
elseif (empty($_POST[g])){
  ...
  mysql_query("INSERT INTO konfirmasi(id_orders,
	...
	if ($_POST[a]=='All'){
			mysql_query("UPDATE laporan SET status_pesanan = 'Menunggu Konfirmasi' where status_pesanan = 'Baru'");						   
		echo "<script>window.alert('Terima Kasih Telah Konfirmasi Pembayaran untuk $_POST[jumlah] Tiket..');
			...
			mysql_query("UPDATE laporan SET status_pesanan = 'Menunggu Konfirmasi' where id_orders='$_POST[a]'");						   
